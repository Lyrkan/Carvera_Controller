    ...
    def _worker_loop(self) -> None:
        ...
        while True:
            ...
            if item is _DISPLAY_FOLLOW:
                ...
                start_vertex, jump_dirty = self._maybe_jump_checkpoint(
                    grid, gen, start_vertex, item.to_vertex
                )
                if jump_dirty:
                    pending_dirty.update(jump_dirty)
                    display_changed_since_cp.clear()
            ...
            if should_emit:
                if not mesh_updates:
                    ...
                    pending_dirty.clear()
                    if self._try_mesh_and_emit(
                        grid, dirty, gen, replace=replace, force_emit=do_flush
                    ):
                        last_emit = now
                        with self._lock:
                            if replace:
